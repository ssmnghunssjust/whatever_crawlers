...
print(ts)
print(salt)
text = 'たぶんさ、向いてるとか向いてないとかじゃないんじゃないかな。失いたくないものがあるときにさ、かっこ悪くても思い切って一歩が踏み出せるかどうかなんだろうね。'.replace(' ', '')
sign = ctx.call('md5', "fanyideskweb" + text + salt + "]BjuETDhU)zqSxf-=B#7m")
print(sign)
...
# resp = session.post('http://fanyi.youdao.com/translate_o?smartresult=dict&smartresult=rule', headers=headers, data=formdata)
resp = session.post('http://fanyi.youdao.com/translate_o?smartresult=dict&smartresult=rule', headers=headers,
                    data=formdata).json()
# target = json.loads(resp.content.decode()).get('translateResult')
# target = json.loads(resp.content.decode()).get('translateResult')[0][0].get('tgt')
target = ''
for sentence in resp.get('translateResult')[0]:
    target += sentence.get('tgt')
print(target)
